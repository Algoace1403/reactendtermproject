...
import { signUpWithEmail, signInWithGoogle, mapAuthError } from "@/services/auth.js";
...
export default function Signup() {
  ...
  return (
    <AuthLayout
      ...
      subtitle="Start a streak today. Takes 30 seconds."
      footer={
        <>
    ...
    >
      <form className="space-y-4" onSubmit={handleSubmit} noValidate>
        ...
        <button type="submit" disabled={submitting} className="btn-primary w-full">
          {submitting ? <Spinner size="sm" label="Creating account…" /> : (<><UserPlus className="h-4 w-4" aria-hidden="true" /> Create account</>)}
        </button>
      </form>
      ...
      <button type="button" onClick={handleGoogle} disabled={googleLoading} className="btn-secondary w-full">
        {googleLoading ? <Spinner size="sm" label="Connecting…" /> : (<><GoogleG /> Sign up with Google</>)}
      </button>
    </AuthLayout>
  );
}

function Field({ id, label, icon, helper, ...rest }) {
  return (
    <div>
